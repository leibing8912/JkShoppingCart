package module.jk.cn.jkshoppingcart.cache;
...
import module.jk.cn.jkshoppingcart.module.ThreadManager;
...
public class SpLocalCache<T> {
	 ...
	 * @return
	 */
	public void save(final Context context, final T data, final CacheCallBackListener localCacheCallBack){
		ThreadManager.getInstance().getNewCachedThreadPool().execute(new Runnable() {
			@Override
			public void run() {
				synchronized (SpLocalCache.class) {
					...
					mHandler.post(new Runnable() {
						...
						public void run() {
							if (localCacheCallBack != null)
								localCacheCallBack.saveCacheComplete();
						}
					});
				}
			}
	...
	private String base64Encode(T data){
		...
		try {
			...
			System.out.println("ddddddddddddddddddddddd e = " + e.getMessage());
			e.printStackTrace();
		}
		return result;
	}
	...
	/**
	 * @className: CacheCallBackListener
	 * @classDescription: 本地缓存回调
	 * @author: leibing
	 * @createTime: 2017/6/5
	 */
	public interface CacheCallBackListener{
		// 保存缓存成功
